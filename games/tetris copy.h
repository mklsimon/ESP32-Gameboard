...
private:
  ...
  int loopMove = 150; // Temps en ms minimum d'un loop - mouvements joueurs
...
public:
    Tetris(int num) : numPlayers(num) {
        ...
        playerMalus.resize(numPlayers, 2); // Malus - lignes a ajouter
        ...
        for (int i = 0; i < numPlayers; i++) {
            ...
            if ( numPlayers < 3){
                ...
                nextPieces[i].position( tetrisBoards[i].positionX + tetrisBoards[i].gameHeight + 4 ,tetrisBoards[i].positionY + 4 );
  ...
  // Boucle de la class
  boolean loop() {
    // Traitement des entrées BT - autant de board que de joueurs
